...
import { Upload, Users, FileText, TrendingUp, Mail, CheckCircle2, XCircle, Loader2, Activity, Clock, UserPlus } from "lucide-react";
...
export default function AdminDashboard() {
    ...
    const [sentInviteStudentIds, setSentInviteStudentIds] = useState<Set<string>>(new Set());
        ...
        try {
            ...
            if (result.ok) {
                ...
                if (data?.success > 0) {
                    ...
                    setSentInviteStudentIds(prev => new Set([...prev, ...idsToSend]));
                ...
                >
                    ...
                    <div className="relative z-10 flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-primary text-md">"Build skills & networks for the future leaders."</p>
                        </div>
                                            ...
                                            {students.map((student) => (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-6 py-4 text-right">
                                                        {!student.profileCompleted && (
                                                            <Button size="sm" onClick={() => handleSendInvites([student.id])} disabled={sendingInvites || invitingStudentIds.has(student.id) || sentInviteStudentIds.has(student.id)} className={sentInviteStudentIds.has(student.id) ? "bg-green-500 text-white hover:bg-green-600" : "bg-primary hover:bg-primary/90 text-white"}>
                                                                {invitingStudentIds.has(student.id) ? (
                                                                    <Loader2 className="h-3 w-3 animate-spin mr-1" />
                                                                ) : sentInviteStudentIds.has(student.id) ? (
                                                                    <CheckCircle2 className="h-3 w-3 mr-1" />
                                                                ) : (
                                                                    <Mail className="h-3 w-3 mr-1" />
                                                                )}
                                                                {invitingStudentIds.has(student.id) ? "Sending..." : sentInviteStudentIds.has(student.id) ? "Sent" : "Send"}
                                                            </Button>
                                                        )}
                                                    </td>
                                                </tr>
